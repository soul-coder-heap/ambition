...
int main()
    try {
        ...
        std::future<std::string> fh = executor.commit([](int n,std::string s)->std::string { 
            std::cout << "hello, fh !  " << std::this_thread::get_id() << std::endl; 
            std::cout <<n<<n<<"--------------------输入参数------------------------"<<s<< std::endl;
            return "hello,fh ret !"; 
        },100000,"baidu.com");
...
*/
        ...
        std::cout << gg.get() << "  " << gh.get()<<" "<<std::this_thread::get_id() << std::endl;;
        std::cout << " =======  sleep ========= " << std::this_thread::get_id() << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(3));
/*
